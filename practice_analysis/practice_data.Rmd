...
```{r descriptives}
df_og%>%group_by(age_grp)%>%summarise(Gender_ratio=paste(sum(gender=="male"), "/", sum(gender=="female")), Education_av=mean(edu_lv),Education_sd=sd(edu_lv), Household_income_av=mean(incm_lv),Household_income_sd=sd(incm_lv), sig_relatives_av=mean(num_relatives),sig_relatives_sd=sd(num_relatives), sig_friends_av=mean(num_friends), sig_friends_sd=sd(num_friends))
...
#Curve fitting
Now that the dataset is tidy, proceed to attemp and fit the hyperbolic discount curve to the group (age_group/)