...
Let's check this:
```{r summary raw values, echo=FALSE, results="hide"}
...
```{r og dataset, echo=FALSE, results="hide"}
...
# where R-square and root-mean-square error (RMSE) were used to assess the goodness of  fit. 
...
data_subset$pred <- discount(V = 48, k = .02,
                             N = data_subset$soc_dist)
...
#AUC- main analysis
The analysis used in the paper is a 2(age group) X 2(donation) X 2(kinship) mixed design, repeated measures ANOVA. The effects found was a main effect of age group such that older adults were less generous than younger adults ($\eta_p ^2$=0.05, F(1,134)=6.52), and a three-way interaction ($\eta_p^2$=0.03, F(1,134)=4.03). I ran the same ANOVA using the r package "ez". 
...
#print(anova_AUC) #-> this run only when all the cells are full (i.e without removing outliers. I'm not sure what to do in case there will be ouliers, as there are ni this simulated data...)
```
...
min(mon_counts$AUC_obs); min(mon_counts$k_obs)#-> check for issues

time_counts=df2%>%filter(donation=="time")%>%group_by(subid)%>%summarise(AUC_obs=length(AUC), k_obs=length(k))%>%ungroup()
min(time_counts$AUC_obs); min(time_counts$k_obs)#-> check for issues.
...
for(i in p_remove_mon){df2_time_crt=df2_time_crt%>%filter(subid!=i)}
...
```{r ANOVA AUC without problem participants}
...
#The t-test of interest is comparing age_group AUC for time donation to relatives. Gong et al. found that older adults were more generous when donating time to relatives compared to young adults.
stats2=ezStats(data = df2_time_crt, wid=.(subid), dv=.(logAUC), within  = .(donation, kinship), between = .(age_grp), type = 3)
stats2
```

The main finding of interest, based on Gong et al's results was that regradless of the age "main" effect, plder adults were more generous than young adults when donating time to relatives.

```{r post hoc test of interest}
...
AUC_plots=ggplot(data = df2_time_crt, aes(x=age_grp, y=log(AUC),color=age_grp))+
  ...
  facet_grid(kinship~donation)+
  ggthemes::theme_few()
ggsave("AUC_plots_simulated_data.png")
AUC_plots

```

## better analysis: mixed linear model with random subject intercepts and planned contrast
```{r lmer}
...
#Run lmer
lmer1=lmer(data=df2, log(AUC)~age_grp*donation*kinship+(1|subid), REML = F) #-> fitting with ML because we are interested in getting accurate estimates fir the parameters.

#Run lmer with covariates
lmer2=lmer(data=df2, log(AUC)~age_grp*donation*kinship+gender+incm_lv+edu_lv+num_friends+num_relatives+(1|subid), REML = F)
...
#Run lmer
lmer3=lmer(data=df2, log(AUC)~age_grp*donation*kinship+(1|subid), REML = F) #-> fitting with ML because we are interested in getting accurate estimates fir the parameters.

#Run lmer with covariates
lmer4=lmer(data=df2, log(AUC)~age_grp*donation*kinship+gender+incm_lv+edu_lv+num_friends+num_relatives+(1|subid), REML = F)
...
```{r k values check for problem participants}
min(mon_counts$k_obs) #-> no problem with money donations
min(time_counts$k_obs) #-> some participants have problematic (+/- 3s.d) time discount rates
...
#k plots
```{r k plots}
#log k values, by age group for each donation*kinship condition
mean_and_ci <- df2_time_crt %>% #-> to get mean log(AUC) values and 95% C.Is in each condition
  group_by(age_grp, donation, kinship) %>% 
  summarise(n = n(),
            mean = mean(log(k)),
...
#Plot
k_plots=ggplot(data = df2_time_crt, aes(x=age_grp, y=log(k),color=age_grp))+
  geom_point(position="jitter",alpha=0.3)+
  stat_summary(fun.y = "mean", geom="bar", alpha=0.3)+
  ylim(-6,-2.7)+
  geom_pointrange(data=mean_and_ci, aes(y=mean,ymin = ci_lower, ymax = ci_upper),color = "black",size=0.7, alpha=0.6)+
