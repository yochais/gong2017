...
anova_AUC=ezANOVA(data = df2, wid=.(subid), dv=.(logAUC), within  = .(donation, kinship), between = .(age_grp), type = 3) #-> using within_full becuase the data isn't balanced, type=3 to emulate SPSS
...
length(p_remove_mon)
ln_t=as.numeric(length(p_remove_time))
df2_mon_crt=df2%>%filter(subid!=one_of(p_remove_mon))
df2_time_crt=df2%>%filter(subid!=p_remove_time[1], subid!=p_remove_time[2], subid!=p_remove_time[3], subid!=p_remove_time[4], subid!=p_remove_time[5], subid!=p_remove_time[6], subid!=p_remove_time[7])

```

```{r ANOVA AUC without problem participants}
anova_AUC2=ezANOVA(data = df2_time_crt, wid=.(subid), dv=.(logAUC), within  = .(donation, kinship), between = .(age_grp), type = 3) #-> using within_full becuase the data isn't balanced, type=3 to emulate SPSS

print(anova_AUC) #->this hacky thing works...
```
