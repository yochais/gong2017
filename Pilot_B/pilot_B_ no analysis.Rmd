...
**Feasability-** Pliot participant completed the task in about 10 minutes. In light of the required sample sizes shown above, acheiving even 80% power would require too large a deviation from the proposed budget. To keep the cost under \$100 allocated to this project, the replication study will have to be powered to observe the stronger effect, that of older adults being more generous in time donation to relatives compared to young adults, in a *one-tailed* test.

###Planned Sample

The above mentioned power calculation suggests that a sample consisting of **39 older adults** (aged 60 or older), and **39 younger adults** (aged 35 or younger) will suffice to achieve 75% power to detect the effect of interest in a one-tailed test (see below). As a note, this is equvilant of having 63% power to observe the effect in a two-tailed test. 

We will rely on Amazon's mechanical Turk for data colelction, and sample only U.S residents to minimize the chance that participants do not read English. Further, older participants will be recruited using the premium qualifications to speed-up data collection and avoid spending a large portion of the budget on non-qualifying participants taking the screener survey.

![](t_test_pwr_75_oneTail.png)
...
Seventh, the age range in the young-adult group in the original paper is 18-44. I am restricting this to be under 35. That is becuase in the literature, people above 35 are typically considered middle aged, or at the very least not "young" adults.
...
```{r prepare data for ANOVA and remove participants with outliers, include=TRUE, results='hide', error=FALSE, warning=FALSE, include=FALSE}
...
```{r post hoc test of interest, include=TRUE, results='hide', error=FALSE, warning=FALSE, include=FALSE}
...
#run t.test
#t.1=t.test(data=df2_tr_crt, log(AUC)~age_grp, alternative='greater') #-> run the one-sided test on the dataframe with only relatives and time reported
#print(t.1)
...
```{r k ANOVA, results='hide', error=FALSE, warning=FALSE, include=FALSE}
...
```{r k values ANOVA, results='hide', error=FALSE, warning=FALSE, include=FALSE}
...
```{r k values post-hoc t-tests, results='hide', error=FALSE, warning=FALSE, include=FALSE}
...
```{r lmer, warning=FALSE, message=FALSE, error=FALSE, results='hide', include=FALSE}
